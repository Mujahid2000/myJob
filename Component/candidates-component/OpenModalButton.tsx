...
const OpenModalButton = ({usersId, resumeId}: {usersId: string, resumeId:string}) => {
    ...
    const {isOpen, userId, resume_Id} = useSelector((state: RootState) => state.profile.positionOpenModal)


    return (
       <div>
      <Button
        onClick={() => dispatch(setOpenPositionModal({ isOpen:true, userId:usersId, resume_Id:resumeId,  }))}
